    ...
    <div className="my-5 overflow-hidden rounded-xl border border-white/10 bg-[#07090d] shadow-2xl shadow-black/20">
      <div className="flex items-center justify-between border-b border-white/[0.07] px-3.5 py-2.5">
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono text-[10px] uppercase tracking-[0.16em] text-white/35">{language || "code"}</span>
        </div>
        <button onClick={copyCode} className="flex items-center gap-1.5 rounded-md px-2 py-1 text-[10px] text-white/35 transition hover:bg-white/[0.05] hover:text-white/70">
  ...
  return (
    <div className="group border-b border-white/[0.045] last:border-0">
      <button
        ...
        className={cn("flex w-full items-start gap-3 px-4 py-3 text-left transition", hasDetails && "hover:bg-white/[0.025]")}
      >
        <EventStatusIcon event={event} />
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <span className="truncate text-[12px] font-medium text-white/75">{event.title}</span>
            <span className="shrink-0 rounded-md border border-white/[0.06] px-1.5 py-0.5 text-[8px] uppercase tracking-[0.14em] text-white/25">{getEventLabel(event)}</span>
          </div>
          <div className="mt-1 flex items-center gap-2 text-[10px] text-white/25">
            {event.agent && <span className="text-blue-400/65">{event.agent}</span>}
            {event.tool && <><ChevronRight size={10} /><span className="font-mono">{event.tool}</span></>}
            {!event.agent && !event.tool && event.description && <span className="truncate">{event.description}</span>}
          </div>
        </div>
        <div className="flex shrink-0 items-center gap-2 text-[9px] text-white/15">
          {formatTime(event.timestamp)}
          {hasDetails && (expanded ? <ChevronDown size={13} /> : <ChevronRight size={13} />)}
        </div>
      </button>
      {expanded && (
        <div className="mx-4 mb-3 ml-13 rounded-lg border border-white/[0.06] bg-black/20 p-3">
          {event.description && <div className="mb-3 text-[11px] leading-5 text-white/45">{event.description}</div>}
          <div className="grid grid-cols-2 gap-3 text-[10px]">
            {event.agent && <div><div className="mb-1 uppercase tracking-wider text-white/20">Agent</div><div className="text-blue-300/65">{event.agent}</div></div>}
            {event.tool && <div><div className="mb-1 uppercase tracking-wider text-white/20">Tool</div><div className="font-mono text-white/45">{event.tool}</div></div>}
            {event.status && <div><div className="mb-1 uppercase tracking-wider text-white/20">Status</div><div className="text-white/45">{event.status}</div></div>}
          </div>
          {event.metadata && Object.keys(event.metadata).length > 0 && (
            <pre className="mt-3 overflow-x-auto rounded-md border border-white/[0.05] bg-black/25 p-2 font-mono text-[9px] leading-5 text-white/30">{JSON.stringify(event.metadata, null, 2)}</pre>
          )}
        </div>
      )}
    </div>
    ...
    <aside className={cn("fixed bottom-4 right-4 z-30 w-[min(390px,calc(100vw-2rem))] overflow-hidden rounded-2xl border border-white/10 bg-[#0a0c10]/95 shadow-2xl shadow-black/50 backdrop-blur-2xl transition-all", !open && "w-auto")}>
      <div className="flex items-center gap-3 px-4 py-3">
        <button type="button" onClick={() => setOpen(!open)} className="flex min-w-0 flex-1 items-center gap-3 text-left">
          <div className="relative flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-blue-500/20 bg-blue-500/10 text-blue-400"><Activity size={15} />{running > 0 && <span className="absolute -right-0.5 -top-0.5 h-1.5 w-1.5 animate-pulse rounded-full bg-blue-400" />}</div>
          {open && <div className="min-w-0"><div className="text-[12px] font-medium text-white/80">Execution trace</div><div className="mt-0.5 flex items-center gap-2 text-[9px] text-white/25"><span>{events.length} events</span>{completed > 0 && <span className="text-blue-400/60">{completed} done</span>}{running > 0 && <span className="text-blue-400">{running} running</span>}{failed > 0 && <span className="text-red-400">{failed} failed</span>}</div></div>}
          {!open && <span className="text-[10px] font-medium text-white/45">{events.length}</span>}
        </button>
        {open && <button type="button" onClick={onClear} aria-label="Clear execution trace" className="rounded-md p-1.5 text-white/25 transition hover:bg-white/[0.05] hover:text-white/60"><Trash2 size={13} /></button>}
        <button type="button" onClick={() => setOpen(!open)} aria-label="Toggle execution trace" className="rounded-md p-1.5 text-white/25 transition hover:bg-white/[0.05] hover:text-white/60">{open ? <ChevronDown size={14} /> : <PanelRight size={14} />}</button>
      </div>
...
export default function App() {
  ...
  const connectSocket = useCallback(() => {
    ...
    socket.onopen = () => {
      if (stoppedRef.current) { socket.close(); return }
    ...
    socket.onmessage = event => {
      try {
        const data = JSON.parse(event.data) as FridayEvent
        if (!data?.id || !data?.type || !data?.title) return
        setEvents(previous => previous.some(item => item.id === data.id) ? previous : [...previous, data])
      } catch { console.warn("[FRIDAY] Invalid execution event") }
    }
    socket.onerror = () => { setSocketConnected(false); setSocketStatus("Connection error") }
    socket.onclose = () => {
      setSocketConnected(false)
      if (stoppedRef.current) { setSocketStatus("Offline"); return }
      setSocketStatus("Disconnected")
      const attempt = reconnectAttemptRef.current
      const delay = Math.min(1000 * 2 ** attempt, 10000)
      reconnectAttemptRef.current = attempt + 1
      reconnectTimerRef.current = setTimeout(() => { reconnectTimerRef.current = null; connectSocket() }, delay)
    }
  }, [])
    ...
    <main className="min-h-screen bg-[#050608] text-white selection:bg-blue-500/30">
      ...
      <header className="sticky top-0 z-40 h-16 border-b border-white/[0.07] bg-[#050608]/85 backdrop-blur-2xl">
        <div className="mx-auto flex h-full max-w-5xl items-center gap-3 px-4 sm:px-6">
          ...
          <div className="flex items-center gap-3">
            <div className="relative flex h-8 w-8 items-center justify-center rounded-xl border border-blue-500/20 bg-blue-500/10"><div className="h-2 w-2 rounded-full bg-blue-400 shadow-[0_0_18px_rgba(59,130,246,.9)]" /></div>
            <div><div className="text-[13px] font-semibold tracking-[0.16em]">FRIDAY</div><div className="hidden text-[9px] tracking-wide text-white/25 sm:block">PERSONAL AI OPERATING ASSISTANT</div></div>
          </div>

          <div className="ml-auto flex items-center gap-2 sm:gap-4">
            <div className="flex items-center gap-1.5 rounded-full border border-white/[0.06] bg-white/[0.02] px-2.5 py-1.5">
              {apiHealthy === false ? <AlertCircle size={11} className="text-red-400" /> : <ShieldCheck size={11} className={apiHealthy ? "text-emerald-400" : "text-white/25"} />}
              <span className="hidden text-[9px] uppercase tracking-wider text-white/30 sm:block">Core</span>
              <span className={cn("h-1.5 w-1.5 rounded-full", apiHealthy === false ? "bg-red-400" : apiHealthy ? "bg-emerald-400" : "bg-white/20")} />
            </div>
            <div className="flex items-center gap-1.5 text-[9px] text-white/25">
              {socketConnected ? <Wifi size={11} className="text-blue-400/80" /> : <WifiOff size={11} />}
              <span className="hidden sm:block">{socketStatus}</span>
            </div>
          ...
          {messages.length === 0 ? (
            <div className="flex flex-1 flex-col items-center justify-center pb-36 pt-20 text-center">
              <div className="relative mb-8">
                ...
                <div className="relative flex h-16 w-16 items-center justify-center rounded-2xl border border-white/10 bg-white/[0.035] shadow-2xl shadow-black/30"><Sparkles size={24} className="text-blue-400" /></div>
              </div>
              ...
              <div className="mt-10 grid w-full max-w-2xl gap-2 sm:grid-cols-3">
                {starterPrompts.map(prompt => (
                  <button key={prompt.label} type="button" onClick={() => setInput(prompt.text)} className="group rounded-xl border border-white/[0.07] bg-white/[0.018] p-4 text-left transition hover:-translate-y-0.5 hover:border-blue-500/20 hover:bg-blue-500/[0.035]">
                    <div className="mb-3 flex h-7 w-7 items-center justify-center rounded-lg bg-white/[0.045] text-white/40 transition group-hover:bg-blue-500/10 group-hover:text-blue-400"><Zap size={13} /></div>
                    <div className="text-[11px] font-medium text-white/55 group-hover:text-white/80">{prompt.label}</div>
                    <div className="mt-1 line-clamp-2 text-[10px] leading-4 text-white/20">{prompt.text}</div>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-10 pb-44 pt-8">
              {messages.map((message, index) => {
                ...
                return (
                  <article key={message.id} className={cn("group", isUser ? "flex justify-end" : "flex justify-start")}>
                    {isUser ? (
                      <div className="max-w-[88%] sm:max-w-[76%]">
                        <div className="rounded-2xl rounded-br-md border border-blue-400/10 bg-blue-600 px-4 py-3 text-[13px] leading-6 text-white shadow-xl shadow-blue-950/20">{message.content}</div>
                        <div className="mt-1.5 text-right text-[9px] text-white/15">{formatTime(message.createdAt)}</div>
                      </div>
                    ) : (
                      <div className="w-full">
                        <div className="mb-3 flex items-center gap-2">
                          <div className="flex h-6 w-6 items-center justify-center rounded-lg border border-blue-500/15 bg-blue-500/[0.07]"><span className="text-[9px] font-bold text-blue-400">F</span></div>
                          <span className="text-[10px] font-medium tracking-[0.14em] text-white/35">FRIDAY</span>
                          <span className="text-[9px] text-white/15">{formatTime(message.createdAt)}</span>
                          <div className="ml-auto flex items-center gap-1 opacity-0 transition group-hover:opacity-100">
                            <button type="button" onClick={() => copyMessage(message)} className="rounded-md p-1.5 text-white/20 hover:bg-white/[0.04] hover:text-white/60">{copiedMessage === message.id ? <Check size={12} /> : <Clipboard size={12} />}</button>
                            {index === messages.length - 1 && !loading && <button type="button" onClick={() => setInput("Please refine your last answer with more detail.")} className="rounded-md p-1.5 text-white/20 hover:bg-white/[0.04] hover:text-white/60"><RotateCcw size={12} /></button>}
                          </div>
                        </div>
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-50 sm:hidden">
          <button type="button" aria-label="Close menu" onClick={() => setSidebarOpen(false)} className="absolute inset-0 bg-black/70 backdrop-blur-sm" />
          <div className="relative h-full w-[280px] border-r border-white/10 bg-[#090b0f] p-5 shadow-2xl">
            <div className="mb-8 flex items-center justify-between"><div className="text-xs font-semibold tracking-[0.18em]">FRIDAY</div><button onClick={() => setSidebarOpen(false)} className="rounded-lg p-2 text-white/30 hover:bg-white/[0.05]"><X size={15} /></button></div>
            <div className="space-y-2">
              <button onClick={clearConversation} className="flex w-full items-center gap-3 rounded-xl border border-white/[0.06] bg-white/[0.025] px-3 py-3 text-left text-xs text-white/55 hover:bg-white/[0.05]"><Plus size={14} />New conversation</button>
              <div className="mt-6 px-1 text-[9px] uppercase tracking-[0.18em] text-white/20">System</div>
              <div className="mt-2 rounded-xl border border-white/[0.06] bg-white/[0.018] p-3"><div className="flex items-center gap-2 text-[10px] text-white/35"><Activity size={12} />Execution events</div><div className="mt-2 text-[9px] text-white/20">{socketConnected ? "Live event stream connected" : "Event stream offline"}</div></div>
            </div>
          </div>
        </div>
      )}
