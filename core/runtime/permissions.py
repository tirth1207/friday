...
def validate_workspace_path(path_str: str, allow_outside: bool = False) -> Path:
    """Validate a path against the active repository workspace boundary."""
    ...
    if not allow_outside:
        try:
            ...
            raise PermissionError(f"Path is outside allowed workspace ({workspace}): {path_str}") from exc

    return target_path


def validate_terminal_command(command: str) -> None:
    """Validate a terminal command string. Block dangerous/destructive commands."""
